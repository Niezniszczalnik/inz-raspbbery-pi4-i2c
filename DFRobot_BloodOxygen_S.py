...
class DFRobot_BloodOxygen_S_i2c(DFRobot_BloodOxygen_S):
    ...
    def read_reg(self, reg_addr, length):
        ...
        '''
        try:
            rslt = self.i2cbus.read_i2c_block_data(
                self.__addr, reg_addr, length)
        except BaseException:
            rslt = -1
        return rslt
...
class DFRobot_BloodOxygen_S_uart(DFRobot_BloodOxygen_S):
  ...
  '''
    ...
    def write_reg(self, reg_addr, data_buf):
        self.write_holding_register(0x20,
                                    int(math.ceil(reg_addr / 2)),
                                    (data_buf[0] << 8) | data_buf[1] & 0xffff)
  ...
  '''
    ...
    def read_reg(self, reg_addr, length):
        return self.read_holding_registers(
            0x20, int(math.ceil(reg_addr / 2)), int(math.ceil(length / 2)))[1:]
